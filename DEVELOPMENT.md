...
### 模板优先原则
- **✅ 修改模板**: 所有UI优化和功能修改都应基于模板文件进行
- **✅ 修改源码**: 修改Java源码文件来调整逻辑和结构
- **❌ 禁止修改**: 除非明确指定，否则不要修改静态生成的HTML文件

### 开发流程
1. **识别问题**: 确定需要修改的功能或UI
2. **定位模板**: 找到对应的模板文件（`bing-template.html`）
3. **修改源码**: 如需要，修改Java源码文件
4. **重新生成**: 运行程序重新生成所有静态文件
5. **验证效果**: 检查所有地区（en-US, zh-CN）的页面

### 文件修改优先级
1. **模板文件**: `docs/bing-template.html` - 最高优先级
2. **Java源码**: `src/main/java/com/wdbyte/bing/` - 次高优先级  
3. **配置文件**: `pom.xml` - 中等优先级
4. **静态文件**: `docs/*.html` - 仅在明确指定时修改

### 生成逻辑
- **Wallpaper.java**: 遍历所有地区，调用WebSiteGenerator
- **WebSiteGenerator**: 使用模板生成所有页面
- **模板修改**: 一次修改，所有页面自动生效
